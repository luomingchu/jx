...
class MController extends BaseController
{
    ...
    public function getVstoreList()
    {
        ...
        // 获取内购列表
        $activities = StoreActivity::where('status', StoreActivity::STATUS_OPEN)->where('deleted', null)->where('store_id', $data->store_id)->where('end_datetime', '>', date('Y-m-d H:i:s'))->oldest()->get();

        if (Input::has('activity_id')) {
            $current_activity = StoreActivity::find(Input::get('activity_id'));
        } else {
        ...
        return View::make('vstore.list')->with(compact('data', 'goods_url', 'activities', 'current_activity'));
    }
    ...
    public function getAppDownLoad()
    {
		...
		// iOS 更新下载页面
        if (Input::has('type') && Input::get('type') == "iOS_self_update") {
            return View::make('appDownLoad.iOS_self_update')->with([
                "iOS_download_url" => $iOS_download_url,
                'enterprise_config' => $enterprise_config,
                'enterprise_id' => $this->enterprise_id
            ]);
