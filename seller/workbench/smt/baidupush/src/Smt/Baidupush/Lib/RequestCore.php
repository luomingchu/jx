...
class RequestCore
{
     ...
     */
    public $request_class = 'Smt\Baidupush\Lib\RequestCore';
    /**
     * The default class to use for HTTP Responses (defaults to <ResponseCore>).
     */
    public $response_class = 'Smt\Baidupush\Lib\ResponseCore';
    /**
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        if ($this->read_stream_read == 0 && isset ($this->seek_position) && $this->seek_position !== ftell($this->read_stream)) {
            if (fseek($this->read_stream, $this->seek_position) !== 0) {
                throw new BaeException ('The stream does not support seeking and is either not at the requested position or the position is unknown.');
            }
        }
            ...
            case self::HTTP_PUT :
                ...
                if (isset ($this->read_stream)) {
                    if (!isset ($this->read_stream_size) || $this->read_stream_size < 0) {
                        throw new BaeException ('The stream size for the streaming upload cannot be determined.');
                    }
    ...
    public function send_request($parse = false)
    {
        ...
        if ($this->response === false) {
            throw new BaeException ('cURL resource: ' . ( string )$curl_handle . '; cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
        }
    ...
    public function send_multi_request($handles, $opt = null)
    {
        ...
        do {
            ...
            while ($done = curl_multi_info_read($multi_handle)) {
                ...
                if ($done ['result'] > 0) {
                    throw new BaeException ('cURL resource: ' . ( string )$done ['handle'] . '; cURL error: ' . curl_error($done ['handle']) . ' (' . $done ['result'] . ')');
