...
class StaffController extends BaseController
{
            ...
            foreach ($excel_data as $key => $item) {
                if ($key >= 1) {
                    ...
                    $validator = Validator::make($inputs, [
                        'real_name' => 'required',
                        'staff_no' => 'required|unique:'.Config::get('database.connections.own.database').'.staffs,staff_no',
                        'mobile' => 'required|unique:'.Config::get('database.connections.own.database').'.staffs,mobile',
                        'store_name' => 'required|exists:'.Config::get('database.connections.own.database').'.store,name',
                        'gender' => 'in:"男","女"',
                        'status' => 'in:"在职","离职"',
    ...
    public function postSave()
    {
        ...
        $validator = Validator::make($inputs, [
            ...
            'mobile' => [
                'mobile',
                'unique:' . Config::get('database.connections.own.database') . '.staffs,mobile,' . Input::get("id", 0)
            ],
            'store_id' => [
                'required',
    ...
    public function delete()
    {
        ...
        $validator = Validator::make(Input::all(), [
            'id' => [
                'required',
                'exists:' . Config::get('database.connections.own.database') . '.staffs,id'
            ]
        ], [
