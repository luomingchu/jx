...
{block breadcrumb}
    ...
    <li><a href="{route('GetCoinLogList')}">指币发放列表</a><span class="divider-last">&nbsp;</span></li>
{/block}
                    ...
                    <table class="table table-striped table-bordered table-hover" id="goods_item_list">
                        ...
                        {foreach $list as $item}
                            <tr>
                                ...
                                <td>{if $item.type eq Coin::TYPE_INCOME}<span style="color: green;">收入{else}<span style="color: red;">支出{/if}</span></td>
                                <td>{$item.amount}</td>
