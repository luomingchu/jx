...
{block breadcrumb}
    ...
    <li><a href="{route('GetRepertoryEnterpriseGoodsList')}">仓库商品列表</a><span class="divider-last">&nbsp;</span></li>
{/block}
                ...
                <div class="widget-body">
                    <div class="row-fluid">
                        <div class="span12 booking-search" style="padding-bottom:5px;">
                            <FORM action="{Route('GetRepertoryEnterpriseGoodsList')}" method="get" id="category_form">
                    ...
                    <table class="table table-striped table-bordered dataTable" id="goods_item_list">
                        ...
                        {foreach $data as $item}
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    ...
                                    <div class="detail_price" style="border-top: 1px solid #ccc;margin-top: 8px;padding-top: 5px;display: none;">
                                        {foreach $item.sku as $stock}
                                            {$stock.sku_string}；价格：{$stock.price}<br/>
                                        {/foreach}
                                    </div>
                                </td>
                                ...
                                <td>
                                    <a class="btn btn-primary" href="{route('EnterpriseGoodsEdit', $item.id)}">编辑</a>
                                    <a class="btn btn-info toggle-status" href="javascript:;" data-id="{$item.id}">上架</a>
                                    <a class="btn btn-danger remove-goods" href="javascript:;" data-id="{$item.id}">删除</a>
                                </td>
                            </tr>
                            {foreachelse}
                            <tr>
                        ...
                        <div class="row-fluid">
                            {if $data->getTotal() > 0}
                                <div class="span6" style="margin-top: 10px;">
                                    ...
                                    <input type="button" id="multiSale" class="btn btn-info" value="批量上架"/>
...
{block script}
    <script>
        ...
        $("#multiRemove").click(function() {
            ...
            iconfirm('确认要删除这'+goods_id.length+'个商品吗？', function() {
                $.ajax({
                    type:'POST',
                    data: { goods_id: goods_id },
                    url: '{action("GoodsController@postDelete")}',
                    dataType: 'text',
        ...
        $("#multiSale").click(function() {
            ...
            iconfirm('确认要上架这'+goods_id.length+'个商品吗？', function() {
                $.ajax({
                    type:'POST',
                    data: { goods_id: goods_id , status:"{Goods::STATUS_OPEN}" },
        ...
        $(".create_sort,.quantity_sort").click(function() {
            var url = "{route('GetRepertoryEnterpriseGoodsList', ['page' => $smarty.get.page, 'name' => $smarty.get.name, 'category_id' => $smarty.get.category_id])}";
