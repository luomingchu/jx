...
//                            $response[$pid]['ios'] = $this->pushIOSMessage($param, $pid);
     ...
     * 推送到所有人
     */
    public function broadcastMassage($content, $params = array(), $message_type = self::MESSAGE_TYPE_TO_APP)
    {
        $param = $params;
     ...
     * 重置平台用户绑定到百度推送的tag分组
     */
    public function resetUserTags($user, $exceptTag = [])
    {
        $info = \DB::table(\Config::get('database.connections.own.database').'.push_bind')->where('member_id', $user->id)->where('kind', $this->kind)->first();
        if (! empty($info)) {
            // 获取用户通过此设备绑定的所有分组
            $bd_bind_tags = $this->channel->queryUserTags($info->push_user_id);
            $diff_tags = ($bd_bind_tags['response_params']['tag_num'] > 0) ? array_diff($exceptTag, array_fetch($bd_bind_tags['response_params']['tags'], 'name')) : $exceptTag;
            if (! empty($bd_bind_tags['response_params']['tags'])) {
                foreach ($bd_bind_tags['response_params']['tags'] as $tag) {
                    if (! empty($exceptTag) && in_array($tag['name'], $exceptTag)) {
                        continue;
                    }
                    $this->deleteTag($tag['name'], $info->push_user_id);
                }
            }
            ...
            if (!empty($diff_tags)) {
                foreach ($diff_tags as $tag) {
                    $optional[Channel::USER_ID] = $info->push_user_id;
                    $this->channel->setTag($tag, $optional);
                }
            }
            \DB::table(\Config::get('database.connections.own.database').'.push_bind')->where('id', $info->id)->update(['tag_list' => implode(',', $exceptTag)]);
        }
    }
