<?php namespace Smt\Baidupush\Lib;
...
class Channel
{
    ...
    public function setApiKey($apiKey)
    {
        ...
        try {
            if ($this->_checkString($apiKey, 1, 64)) {
                ...
                throw new BaeException ("invaid apiKey ( ${apiKey} ), which must be a 1 - 64 length string", self::CHANNEL_SDK_INIT_FAIL);
            }
        } catch (BaeException $ex) {
    ...
    public function setSecretKey($secretKey)
    {
        ...
        try {
            if ($this->_checkString($secretKey, 1, 64)) {
                ...
                throw new BaeException ("invaid secretKey ( ${secretKey} ), which must be a 1 - 64 length string", self::CHANNEL_SDK_INIT_FAIL);
            }
        } catch (BaeException $ex) {
            $this->_channelExceptionHandler($ex);
            return false;
    ...
    public function setCurlOpts($arr_curlOpts)
    {
        ...
        try {
            if (is_array($arr_curlOpts)) {
                ...
                throw new BaeException('invalid param - arr_curlOpts is not an array ['
    ...
    public function setHost($host)
    {
        ...
        try {
            if ($this->_checkString($host, 1, 1024)) {
                ...
                throw new BaeException ("invaid host ( ${host} ), which must be a 1 - 1024 length string", self::CHANNEL_SDK_INIT_FAIL);
            }
        } catch (BaeException $ex) {
            $this->_channelExceptionHandler($ex);
            return false;
            ...
            switch ($pushType) {
                case self::PUSH_TO_USER:
                    if (!array_key_exists(self::USER_ID, $arrArgs) || empty($arrArgs[self::USER_ID])) {
                        throw new BaeException("userId should be specified in optional[] when pushType is PUSH_TO_USER", self::CHANNEL_SDK_PARAM);
                    }
                    break;

                case self::PUSH_TO_TAG:
                    if (!array_key_exists(self::TAG_NAME, $arrArgs) || empty($arrArgs[self::TAG_NAME])) {
                        throw new BaeException("tag should be specified in optional[] when pushType is PUSH_TO_TAG", self::CHANNEL_SDK_PARAM);
                    }
                    break;
                ...
                default:
                    throw new BaeException("pushType($pushType) must be in range[1,3]", self::CHANNEL_SDK_PARAM);
            }
     ...
     * @throws BaeException 如果出错，则抛出异常，异常号是self::CHANNEL_SDK_INIT_FAIL
     */
    public function __construct($apiKey = NULL, $secretKey = NULL, $arr_curlOpts = array())
    {
        if ($this->_checkString($apiKey, 1, 64)) {
            ...
            throw new BaeException("invalid param - apiKey[$apiKey],"
        ...
        if ($this->_checkString($secretKey, 1, 64)) {
            ...
            throw new BaeException("invalid param - secretKey[$secretKey],"
                . "which must be a 1 - 64 length string",
                self::CHANNEL_SDK_INIT_FAIL);
        }

        if (!is_array($arr_curlOpts)) {
            throw new BaeException('invalid param - arr_curlopt is not an array ['
                . print_r($arr_curlOpts, true) . ']',
                self::CHANNEL_SDK_INIT_FAIL);
        }
        $this->_curlOpts = $this->_curlOpts + $arr_curlOpts;
     ...
     * @throws BaeException 如果出错，则抛出BaeException异常，异常类型为self::CHANNEL_SDK_PARAM
     *
     * @version 1.0.0.0
     */
                               ...
                               $throw = true)
    {
        ...
        if ($throw) {
            throw new BaeException('no param (' . $dis[$opt_key] . ') was found',
     ...
     * @access protected
     * @param array $opt 参数数组
     * @throws BaeException 如果出错，则抛出异常，异常号为 self::CHANNEL_SDK_PARAM
     *
     * @version 1.0.0.0
     */
    protected function _adjustOpt(&$opt)
    {
        if (!isset($opt) || empty($opt) || !is_array($opt)) {
            throw new BaeException('no params are set', self::CHANNEL_SDK_PARAM);
        }
    ...
    {
        if (!is_array($params)) {
            throw new BaeException('no params', self::CHANNEL_SDK_PARAM);
        }
        foreach ($params as $key => $value) {
            switch ($key) {
                case self::USER_ID:
                    if (!is_string($value)) {
                        throw new BaeException("USER_ID($value) is not string",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::CHANNEL_ID:
                    if (!is_numeric($value)) {
                        throw new BaeException("CHANNEL_ID($value) is not numeric",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::DEVICE_TYPE:
                    if (!is_numeric($value) || $value < 0 || $value > 5) {
                        throw new BaeException("invalid DEVICE_TYPE($value)",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::TAG_NAME:
                    if (!is_string($value) || strlen($value) > 128) {
                        throw new BaeException("TAG_NAME($value) must be a string and strlen <= 128",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::MESSAGE_TYPE:
                    if (!is_numeric($value) || $value < 0 || $value > 1) {
                        throw new BaeException("invalid MESSAGE_TYPE($value) must be 0 or 1",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::NAME:
                    if (!is_string($value) || strlen($value) > 128) {
                        throw new BaeException("IOS_CERT_NAME($value) must be a string and strlen <= 128",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
                case self::DESCRIPTION:
                    if (!is_string($value) || strlen($value) > 256) {
                        throw new BaeException("IOS_CERT_DESCRIPTION($value) must be a string and strlen <= 256",
                            self::CHANNEL_SDK_PARAM);
                    }
                    break;
            }
     ...
     * @access protected
     * @param array $opt 参数数组
     * @throws BaeException 如果出错，则抛出异常，错误号为self::CHANNEL_SDK_SYS
     *
     * @version 1.0.0.0
     */
     ...
     * @throws BaeException 如果出错，则抛出异常
     *
     * @version 1.0.0.0
     */
    protected function _commonProcess($paramOpt = NULL)
    {
        ...
        if (empty($ret)) {
            throw new BaeException('base control returned empty object',
     ...
     * @throws BaeException 如果出错，则抛出异常，异常号为self::Channel_SDK_PARAM
     *
     * @version 1.0.0.0
     */
    protected function _mergeArgs($arrNeed, $tmpArgs)
    {
        ...
        if (count($tmpArgs) - 1 != count($arrNeed) && count($tmpArgs) != count($arrNeed)) {
            ...
            $keys .= ')';
            throw new BaeException('invalid sdk params, params' . $keys . 'are needed',
                self::CHANNEL_SDK_PARAM);
        }
        ...
        if (count($tmpArgs) - 1 == count($arrNeed) && !is_array($tmpArgs[count($tmpArgs) - 1])) {
            throw new BaeException('invalid sdk params, optional param must be an array',
                self::CHANNEL_SDK_PARAM);
        }
        ...
        if (!is_null($arrNeed)) {
            foreach ($arrNeed as $key) {
                if (!is_integer($tmpArgs[$idx]) && empty($tmpArgs[$idx])) {
                    throw new BaeException("lack param (${key})", self::CHANNEL_SDK_PARAM);
                }
